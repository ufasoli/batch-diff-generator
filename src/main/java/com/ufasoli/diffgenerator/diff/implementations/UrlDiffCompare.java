...
/**
 * Class Name   : UrlDiffCompare Description  : Date         : 28 janv. 2013 Copyright    : ARTE G.E.I.E.      Auteur       : Ulises Fasoli
 */
public class UrlDiffCompare {
	...
	public void compare() {
		...
		List<DiffRow> rows = drg.generateDiffRows(url1Lines, url2Lines);
		...
		try {
			...
			htmlDiffbody.append(String.format(
					"<tr><th>Line</th><th>%s</th><th>Line</th><th>%s</th></tr>", url1, url2));
			...
			htmlHead.append(String
					.format("<ul><li>Inserted Lines : %s</li><li>Deleted Lines : %s</li><li>Modified Lines : %s</li><li>Non Modified lines :%s </li></ul> <hr />",
							inserted, deleted, changes, equal));
	...
	private List<String> urlToLines(String stringUrl) {
		...
		try {
			...
			if (urlLines.size() == 1) {
				...
				FileWriter tmpFile = new FileWriter(outputFolder + "tmp.json");
				ObjectMapper mapper = new ObjectMapper();
				JsonNode rootNode = mapper.readValue(urlLines.get(0), JsonNode.class);
				...
				tmpFile.close();
				urlLines.clear();
				urlLines.addAll(fileToLines(new File(outputFolder + "tmp.json")));
			}
